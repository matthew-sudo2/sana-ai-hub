...
# ---------------------------------------------------------------------------
# Visualization Explanation Schema  (for frontend rendering)
# ---------------------------------------------------------------------------
...
class VisualizationExplanation(BaseModel):
    """Structured explanation for a visualization, generated by Analyst Agent."""
    summary: str                    # 1-2 sentences: what the chart shows
    insight: str                    # 1-2 sentences: key finding
    interpretation: str             # 2-3 sentences: what it means
    suggestion: str                 # 1-2 sentences: recommended action
...
# ---------------------------------------------------------------------------
# Visualization Explanation Generation  (for frontend rendering)
# ---------------------------------------------------------------------------

def _load_analyst_prompt_template(template_path: Path) -> str:
    """Load the prompt template from file."""
    if not template_path.exists():
        # Fallback template if file not found
        return """You are a data visualization analyst. Your job is to explain charts to non-technical users.

Generate a structured explanation with:
- summary: 1-2 sentences on what the chart shows
- insight: 1-2 sentences on the key finding
- interpretation: 2-3 sentences on what it means
- suggestion: 1-2 sentences on recommended action

Chart: {chart_type}, X: {x}, Y: {y}

Return ONLY valid JSON with these keys: summary, insight, interpretation, suggestion."""
    
    try:
        return template_path.read_text(encoding="utf-8")
    except Exception as e:
        print(f"[analyst] Warning: could not load prompt template: {e}", flush=True)
        return ""


def _generate_explanation_cache_key(dataset: str, chart_type: str, x: str, y: str) -> str:
    """Generate a stable hash key for caching explanations."""
    cache_input = f"{dataset}|{chart_type}|{x}|{y}".encode("utf-8")
    return hashlib.md5(cache_input).hexdigest()


def _generate_visualization_explanation(
    chart_type: str,
    title: str,
    x: str,
    y: str,
    ...
    """
    Generate a human-readable explanation for a visualization using LLM.
    
    Parameters
    ----------
    chart_type : histogram, scatter, line, bar, etc.
    title : chart title
    ...
    Returns
    -------
    VisualizationExplanation or None if explanation generation fails
    """
    if not _ollama_alive(ollama_host):
        print(f"[analyst] LLM unavailable at {ollama_host}, skipping explanation", flush=True)
        return None
    
    if template_path is None:
        template_path = Path(__file__).parent.parent / "prompts" / "analyst_prompt.txt"
    
    template = _load_analyst_prompt_template(template_path)
    if not template:
        return None
    ...
    try:
        ...
        print(f"[analyst] Prompt formatting failed: {e}", flush=True)
        return None
    
    # Call LLM
    try:
        raw_response = _ollama_generate(model=model, prompt=prompt, host=ollama_host)
        if not raw_response:
            return None
        ...
        if start == -1 or end == -1:
            print(f"[analyst] No JSON found in LLM response", flush=True)
            return None
        ...
        required_fields = {"summary", "insight", "interpretation", "suggestion"}
        if not all(field in explanation_dict for field in required_fields):
            print(f"[analyst] LLM response missing required fields", flush=True)
            return None
        
        return VisualizationExplanation(
            summary=str(explanation_dict.get("summary", "")),
            insight=str(explanation_dict.get("insight", "")),
            interpretation=str(explanation_dict.get("interpretation", "")),
            suggestion=str(explanation_dict.get("suggestion", "")),
        )
    except json.JSONDecodeError as e:
        print(f"[analyst] Failed to parse LLM response as JSON: {e}", flush=True)
        return None
    except Exception as e:
        print(f"[analyst] Explanation generation failed: {e}", flush=True)
        return None
...
def _load_cached_explanation(
    ...
    cache_key = _generate_explanation_cache_key(dataset, chart_type, x, y)
    cache_file = cache_dir / f"explanation_{cache_key}.json"
    
    if cache_file.exists():
        try:
            cached = json.loads(cache_file.read_text(encoding="utf-8"))
            return VisualizationExplanation(**cached)
        except Exception as e:
            print(f"[analyst] Failed to load cached explanation: {e}", flush=True)
    
    return None


def _save_cached_explanation(
    cache_dir: Path,
    explanation: VisualizationExplanation,
    chart_type: str,
    x: str,
    ...
    try:
        ...
        print(f"[analyst] Failed to cache explanation: {e}", flush=True)
